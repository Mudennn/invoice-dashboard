...
@section('content')
    <div style="padding: 40px;">
        <div class="d-flex justify-content-between align-items-center">
            <h2 class="text-2xl font-bold">Self-Billed Invoices</h2>
            <a href="{{ route('self_billed_invoices.create') }}" class="primary-button">Create</a>
        </div>
        <div class="table-responsive" style="min-height: 200px; overflow-y: auto; margin-top: 32px;">
            <table class="table table-hover table-bordered align-middle text-nowrap" id="invoiceTable">
                ...
                <tbody>
                    @foreach ($self_billed_invoices as $index => $invoice)
                        <tr>
                            <td class="text-center">{{ $index + 1 }}</td>
                            <td>{{ $invoice->self_billed_invoice_no }}</td>
                            <td>{{ $invoice->self_billed_invoice_date }}</td>
                            <td>{{ $invoice->customer }}</td>
                            <td>RM{{ number_format($invoice->selfBilledInvoiceItems->first()->subtotal ?? 0, 2) }}</td>
                            @php
                            ...
                            <td class="text-center">
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                                        <li><a href="{{ route('self_billed_invoices.edit', $invoice->id) }}" class="dropdown-item"
                                                href="#"
                                        ...
                                        <li><a href="{{ route('self_billed_invoices.view', $invoice->id) }}" class="dropdown-item"
                                                style="display: flex; justify-content: start; align-items: center; gap:8px; font-size:14px;"><span
                                                    class="material-symbols-outlined" style="font-size: 14px">
                                                    list_alt
                                                </span> View Items</a></li>
                                        <li><a href="{{ route('self_billed_invoices.print', $invoice->id) }}" class="dropdown-item"
                                                    target="_blank"
                                                    style="display: flex; justify-content: start; align-items: center; gap:8px; font-size:14px;"><span
                                                        class="material-symbols-outlined" style="font-size: 14px">
                                                        print
                                                    </span> Print Invoice</a></li>
                                        <li><a href="{{ route('self_billed_invoices.destroy', $invoice->id) }}"
